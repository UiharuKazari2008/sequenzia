...
;(function($, undefined) {
	...
	var Public = function(url, replace, state) {

		var event = $.Event((replace) ? 'replace' : 'push');
		...
		if (_type === 'pathname') {
			...
			if (_last !== url)
				...
				if (replace)
					window.history.replaceState((state) ?  state : {}, null, _last = url);
				else
					window.history.pushState((state) ?  state : {}, null, _last = url);
		...
		Public.context.trigger((replace) ? 'replaced' : 'pushed', [url, _type]);
	...
	Public.getSupports = function(type) {
		...
		if (!size || type === 'replaceState')

			result.replaceState = ('replaceState' in window.history);
