...
function sendLEDValues(values) {
    if (remoteWACCALED && values.length > 1) {
        $.ajax({
            async: true,
            url: `http://${remoteWACCALED}/setLED?ledBrightness=${(_night) ? '64' : '128'}&ledValues=${values}&driveMode=1&driveSettings=18`,
            type: "GET", data: '',
            processData: false,
            contentType: false,
            headers: {
...
function sendLEDStatic(values) {
    if (remoteWACCALED) {
        $.ajax({
            async: true,
            url: `http://${remoteWACCALED}/setLEDColor?ledBrightness=50&ledColor=${values}&driveMode=1`,
            type: "GET", data: '',
            processData: false,
            contentType: false,
            headers: {
