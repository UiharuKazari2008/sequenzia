...
function reorderColors(colors) {
    let reorderedColors = [];
    const ringCount = 8;
    const colorsPerRing = colors.length / ringCount;
    ...
    for (let i = 0; i < colorsPerRing; i++) {
        for (let j = 0; j < ringCount; j++) {
            const colorIndex = j * colorsPerRing + i;
            const [r, g, b] = hexToRgb(colors[colorIndex]); // Assuming you have a hexToRgb function
            let [h, s, l] = rgbToHsl(r, g, b);
            l = Math.max(0, l - 0.1); // Decrease lightness by 10%
            const [newR, newG, newB] = hslToRgb(h, s, l);
            const newHex = rgbToHex(newR, newG, newB); // Assuming you have a rgbToHex function
            reorderedColors.push(newHex);
        }
    }
...
function sendLEDValues(values) {
    if (remoteWACCALED && values.length > 1) {
        $.ajax({
            async: true,
            url: `http://${remoteWACCALED}/setLED?ledBrightness=${(_night) ? '128' : '255'}&ledValues=${values}`,
            type: "GET", data: '',
            processData: false,
            contentType: false,
            headers: {
