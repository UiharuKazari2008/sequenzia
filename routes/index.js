...
router.use('/parity', sessionVerification, handleExchange, readValidation, async (req, res) => {
    try {
        ...
        if (params.length > 0 && config.sbi_interfaces && config.sbi_interfaces.discord) {
            ...
            const request = http.get(`http://${discord_host}/get/spanned_file?uuid=${params[0]}${(global.proxy_host) ? '&proxy=' + encodeURIComponent(global.proxy_host) : ''}`, async function (response) {
                response.pipe(res);
            });
