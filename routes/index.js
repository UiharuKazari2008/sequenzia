...
router.use('/avatars', async function (req, res) {
    try {
        ...
        if (params.length === 2) {
            ...
            res.status(400).send('Missing Parameters');
            printLine('ProxyMedia', `Invalid Request to proxy, missing a ID`, 'error');
        }
    } catch (err) {
        res.status(500).json({
            state: 'HALTED',
...
router.use('/icons', async function (req, res) {
    try {
        const params = req.path.substr(1, req.path.length - 1).split('/')
        const query = Object.entries(req.query).map((k) => k[0] + '=' + k[1] ).join('&')
