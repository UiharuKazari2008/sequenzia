                ...
                if (err) {
                    ...
                    async function returnContent(url) {
                        if (url) {
                            ...
                            const bitmap = readFileSync(path.join(global.fw_serve, `.${message.fileid}`));
                            const contentType = mime.lookup(path.join(global.fw_serve, `.${message.fileid}`))
                    ...
                    if (params[0] === 'full' || params[0] === 'full64') {
                        if (message.filecached && global.fw_serve) {
                            returnContent();
...
router.use('/*', function(req, res){
    if (req.session && req.session.loggedin) {
        res.status(404).send('Not Implemented - CDS Audit');
    } else {
