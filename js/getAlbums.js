            ...
            if (query && query.rows && query.rows.length > 0) {
                ...
                if (req.query.json && req.query.json === 'true') {
                    ...
                    const rows = album_rows.map(e => {
                        let ranImage = (e.cdn_host !== null && config.local_cdn_list.filter(e => e.id === e.cdn_host).length > 0 && e.preview_hint) ? `${config.local_cdn_list.filter(e => e.id === e.cdn_host)[0].access_url}preview/${e.path_hint}/${e.preview_hint}` : ( e.cache_proxy) ? e.cache_proxy.startsWith('http') ? e.cache_proxy : `https://media.discordapp.net/attachments${e.cache_proxy}` : (e.attachment_hash && e.attachment_name) ? `https://media.discordapp.net/attachments/` + ((e.attachment_hash.includes('/')) ? e.attachment_hash : `${e.channel}/${e.attachment_hash}/${e.attachment_name}`) : undefined
                        return {
                            ...e,
