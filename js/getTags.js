...
module.exports = async (req, res, next) => {
    ...
    if (!thisUser) {
        ...
        if (req.headers['x-requested-page'] && req.headers['x-requested-page'] === 'SeqPaginator' ) {
            ...
            const sqlResult = await sqlPromiseSimple(`SELECT rec.*, cdn.host AS cdn_host, cdn.full_hint, cdn.preview_hint, cdn.ext_0_hint FROM (${sqlCall} LIMIT ${limit} OFFSET ${offset}) rec LEFT OUTER JOIN (${selectCDN}) cdn ON (rec.eid = cdn.eid)`)
