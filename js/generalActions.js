        ...
        if (thisUser.user && (req.session.login_source < 900 || req.body.action === "GetLastHistoryPage" || (req.body.action && (req.body.action === "Pin" || req.body.action === "SetWatchHistory") && req.body && req.body.bypass && req.body.bypass === "appIntent"))) {
            switch (req.body.action) {
                ...
                case 'CollItemAddBulk':
                    if (!req.body.messagelist || (req.body.messagelist && req.body.messagelist.length === 0)) {
                        res.status(400).send(`Missing array of messages for bulk action`);
                    } else {
                        sqlSafe(`SELECT DISTINCT eid FROM sequenzia_album_items, sequenzia_albums WHERE sequenzia_album_items.aid = ? AND sequenzia_albums.aid = sequenzia_album_items.aid AND sequenzia_albums.owner = ? AND eid IN (${req.body.messagelist.join(', ')})`, [req.body.albumid, thisUser.discord.user.id], async (err, found) => {
                            if (err) {
                                ...
                                const existingItems = found.map(e => e.eid.toString());
