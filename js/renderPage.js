        ...
        if (results.results) {
            results.results.forEach(item => {
                ...
                if (item.entities.download && item.entities.download.length > 5) {
                    ...
                    podcastItem.enclosure = {
                        url: `${req.protocol}://${req.get('host')}${(req.port) ? ':' + req.port : ''}/stream/${item.entities.meta.fileid}/${item.entities.filename}?blind_key=${req.session.discord.user.token_login}`
                    }
                }
