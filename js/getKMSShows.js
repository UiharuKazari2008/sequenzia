...
module.exports = async (req, res, next) => {
    ...
    if (!req.session.discord) {
        ...
        let sqlCall = `SELECT * FROM (SELECT kms.kms_series_name, kms.series_data, kms.group_name, kms.icon, kms.background, kms.poster FROM (SELECT ${sqlFields} FROM ${sqlTables} WHERE ((${baseQ}) AND (kanmi_records.channel = ${req.session.cache.channels_view}.channelid))) main_records LEFT OUTER JOIN (${kongouShows}) kms ON (kms.eid = main_records.eid) WHERE kms_series_name IS NOT NULL GROUP BY kms.show_id) x ORDER BY x.kms_series_name`;
