...
module.exports = async (req, res, next) => {
    ...
    if (!thisUser) {
        ...
        // Main Query
        let baseQ = ''
        let sqlOrder = 'kms_list.kms_series_name'
        let media_group = ''
        if (req.query.group) {
        ...
        if (req.query.sort === 'name') {
            sqlOrder = `kms_list.kms_series_name ${(req.query.reverse === 'true') ? 'DESC' : ''}`
        } else if (req.query.sort === 'release') {
            sqlOrder = `kms_data.date  ${(req.query.reverse === 'true') ? '' : 'DESC'}`
        } else {
            sqlOrder = `kms_list.leid  ${(req.query.reverse === 'true') ? '' : 'DESC'}`
        }
        ...
        const selectCDN = `SELECT * FROM kanmi_aux_cdn WHERE ${(config.local_cdn_list && config.local_cdn_list.length > 0) ? 'AND (' + config.local_cdn_list.map(e => 'host = ' + e.id).join(' OR ') + ')' : ''}`
        const kongouShows = `SELECT rec.*, cdn.host AS cdn_host, cdn.dat_0_hint, cdn.dat_1_hint FROM (SELECT kms_ep.eid, kms_show.group_name, kms_show.description, kms_show.icon, kms_show.show_id, kms_ep.episode_num, kms_ep.episode_name AS kms_episode_name, kms_ep.season_num, kms_show.name AS kms_series_name, kms_show.type AS group_type, kms_show.original_name AS kms_series_original_name, kms_show.nsfw AS series_nsfw, kms_show.group_nsfw, kms_show.subtitled AS series_subtitled, kms_show.background, kms_show.poster FROM (SELECT eid, show_id, episode_num, episode_name, season_num, background, still FROM kongou_episodes) kms_ep LEFT OUTER JOIN (SELECT * FROM (SELECT show_id, media_group, name, original_name, nsfw, subtitled, genres, background, poster FROM kongou_shows ${(media_group.length > 0) ? "WHERE media_group = '" + media_group + "'" : ''}) s LEFT JOIN (SELECT media_group AS group_id, adult AS group_nsfw, type, name AS group_name, description, icon FROM kongou_media_groups) g ON (s.media_group = g.group_id)) kms_show ON (kms_ep.show_id = kms_show.show_id)) rec LEFT OUTER JOIN (${selectCDN}) cdn ON (rec.show_id = cdn.record_int)`

        let sqlCall = `SELECT records.*, json.series_data FROM (SELECT kms_list.* FROM (SELECT COUNT(main_records.eid) AS ieid, MAX(main_records.eid) AS leid, kms.kms_series_name, kms.group_name, kms.icon, kms.series_nsfw, kms.group_nsfw, kms.group_type, kms.series_subtitled, kms.background, kms.poster, kms.show_id FROM (SELECT ${sqlFields} FROM ${sqlTables} WHERE (kanmi_records.channel = ${thisUser.master.cache.channels_view}.channelid${(media_group.length > 0) ? " AND " + thisUser.master.cache.channels_view + ".media_group = '" + media_group + "'" : ''})) main_records LEFT OUTER JOIN (${kongouShows}) kms ON (kms.eid = main_records.eid) GROUP BY kms.show_id) kms_list LEFT OUTER JOIN (SELECT show_id, STR_TO_DATE( JSON_UNQUOTE(JSON_EXTRACT(data, '$.date')), "%Y-%m-%d") AS date FROM kongou_shows) kms_data ON (kms_list.show_id = kms_data.show_id) ORDER BY ${sqlOrder} LIMIT ${sqllimit + 10} OFFSET ${offset}) records LEFT JOIN (SELECT show_id, data AS series_data FROM kanmi_system.kongou_shows) json ON (records.show_id = json.show_id)`;
            ...
            if (messageResults && messageResults.rows.length > 0) {
                ((messages) => {
                    ...
                    if (resultsArray.length > 0) {
                        ...
                        printLine('GetData', `"${thisUser.master.discord.user.username}" => "KMS Media Browser" - ${resultsArray.length} Returned (${_req_uri})`, 'info', {
                            title: pageTitle,
                            full_title: pageFullTitle,
                            page_image: null,
                            description: '',
                            ...
                            unmatched_found: (messages.filter(e => e.kms_series_name === null).length > 0),
                            media_group: req.query.group,
                            prevurl: prevurl,
                            nexturl: nexturl,
                            req_limit: limit,
