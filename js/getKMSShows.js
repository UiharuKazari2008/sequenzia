...
module.exports = async (req, res, next) => {
    ...
    if (!req.session.discord) {
        ...
        let sqlCall = `SELECT kms_list.*, kms_data.series_data FROM (SELECT COUNT(main_records.eid) AS ieid, kms.kms_series_name, kms.group_name, kms.icon, kms.series_nsfw, kms.series_subtitled, kms.background, kms.poster, kms.show_id FROM (SELECT ${sqlFields} FROM ${sqlTables} WHERE (kanmi_records.channel = ${req.session.cache.channels_view}.channelid${(media_group.length > 0) ? " AND " + req.session.cache.channels_view + ".media_group = '" + media_group + "'" : ''})) main_records LEFT OUTER JOIN (${kongouShows}) kms ON (kms.eid = main_records.eid) GROUP BY kms.show_id) kms_list LEFT OUTER JOIN (SELECT show_id, data AS series_data FROM kongou_shows) kms_data ON (kms_list.show_id = kms_data.show_id) ORDER BY kms_list.kms_series_name`;
