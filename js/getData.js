    ...
    if (req.session.active_exchange && req.session.active_exchange !== 'master') {
        ...
        let sqlCall = (() => {
            ...
            if (req.query.fast_query && req.query.fast_query === '1') {
                return `SELECT * FROM (${selectBase}) base ${sqlFavJoin} (${selectFavorites} ${(req.query && req.query.cached && req.query.cached === 'true') ? "WHERE (full = 1 OR mfull = 1)" : ""}) fav ON (base.eid = fav.fav_id)${(sqlFavWhere.length > 0) ? 'WHERE ' + sqlFavWhere.join(' AND ') : ''}`;
            } else if (req.query.fast_query && req.query.fast_query === '2') {
                return `${selectBase}`;
            } else {
                return `SELECT * FROM (SELECT * FROM (${selectBase}) base ${sqlFavJoin} (${selectFavorites} ${(req.query && req.query.cached && req.query.cached === 'true') ? "WHERE (full = 1 OR mfull = 1)" : ""}) fav ON (base.eid = fav.fav_id)${(sqlFavWhere.length > 0) ? 'WHERE ' + sqlFavWhere.join(' AND ') : ''}) i_wfav ${sqlHistoryJoin} (SELECT * FROM (${selectHistory}) hist LEFT OUTER JOIN (${selectConfig}) conf ON (hist.history_name = conf.config_name)) his_wconf ON (i_wfav.eid = his_wconf.history_eid)${sqlHistoryWherePost}${(req.query && req.query.displayname && req.query.displayname === '*' && req.query.history  && req.query.history === 'only') ? ' WHERE config_show = 1 OR config_show IS NULL' : ''}`;
            }
        })();
        let sqlCallNoPreLimit = (() => {
            if (req.query.sort === 'rating' || req.query.sort === 'rating_count' || req.query.min_tags || req.query.max_tags || req.query.min_score || req.query.max_score)
                return `SELECT * FROM (SELECT base_full.*, trate.tag_count, trate.tag_num FROM (SELECT * FROM (${selectBaseNoPreLimit}) base ${sqlFavJoin} (${selectFavorites} ${(req.query && req.query.cached && req.query.cached === 'true') ? "WHERE (full = 1 OR mfull = 1)" : ""}) fav ON (base.eid = fav.fav_id)${(sqlFavWhere.length > 0) ? 'WHERE ' + sqlFavWhere.join(' AND ') : ''}) base_full LEFT JOIN (SELECT DISTINCT eid, COUNT(eid) AS tag_num, SUM(rating) AS tag_count FROM sequenzia_index_matches GROUP BY eid) trate ON (base_full.eid = trate.eid AND trate.tag_count <= 100)) i_wfav ${sqlHistoryJoin} (SELECT * FROM (${selectHistory}) hist LEFT OUTER JOIN (${selectConfig}) conf ON (hist.history_name = conf.config_name)) his_wconf ON (i_wfav.eid = his_wconf.history_eid)${sqlHistoryWherePost}${(req.query && req.query.displayname && req.query.displayname === '*' && req.query.history  && req.query.history === 'only') ? ' WHERE config_show = 1 OR config_show IS NULL' : ''}`;
            if (req.query.fast_query && req.query.fast_query === '1') {
                return `SELECT * FROM (${selectBaseNoPreLimit}) base ${sqlFavJoin} (${selectFavorites} ${(req.query && req.query.cached && req.query.cached === 'true') ? "WHERE (full = 1 OR mfull = 1)" : ""}) fav ON (base.eid = fav.fav_id)${(sqlFavWhere.length > 0) ? 'WHERE ' + sqlFavWhere.join(' AND ') : ''}`;
            } else if (req.query.fast_query && req.query.fast_query === '2') {
                ...
                randomImage.forEach(function(image) {
                    ...
                    if ((page_uri === '/ambient-refresh' || page_uri === '/ambient-remote-refresh') && !(req.query && req.query.nocds && req.query.nocds === 'true')) {
                        ...
                        if (image.cdn_host !== null && config.local_cdn_list.filter(e => e.id === image.cdn_host).length > 0 && image.full_hint) {
                            ...
                            ranfullImage = `${(req.session && req.session.lite_mode === true) ? '/attachments' : 'https://cdn.discordapp.com/attachments'}/` + ((image.attachment_hash.includes('/')) ? image.attachment_hash : `${image.channelid}/${image.attachment_hash}/${image.attachment_name}`)
                        }
                    }
                            ...
                            if (index + 1 <= limit) {
                                ...
                                if (item.attachment_extra !== null) {
                                    ...
                                    if ( item.cdn_host !== null && config.local_cdn_list.filter(e => e.id === item.cdn_host).length > 0 && item.preview_hint) {
                                        ...
                                        imageurl = `${(req.session && req.session.lite_mode === true) ? '/media_attachments' : 'https://media.discordapp.net/attachments'}/` + ((item.attachment_hash.includes('/')) ? `${item.attachment_hash}${getimageSizeParam()}` : `${item.channel}/${item.attachment_hash}/${item.attachment_name}${getimageSizeParam()}`)
                                    }
                                    let advColor = [];
