...
module.exports = async (req, res, next) => {
    ...
    if (req.session.active_exchange && req.session.active_exchange !== 'master') {
        ...
        const selectBase = `SELECT rec.*, cdn.host AS cdn_host, path_hint, cdn.full_hint, cdn.preview_hint, cdn.ext_0_hint FROM (SELECT x.*, y.data FROM (SELECT ${sqlFields.join(', ')} FROM ${sqlTables.join(', ')} WHERE (${execute} AND (${sqlWhere.join(' AND ')}))` + ((sqlorder.trim().length > 0 && enablePrelimit) ? ` ORDER BY ${sqlorder}` : '') + ((enablePrelimit) ? ` LIMIT ${sqllimit + 10} OFFSET ${offset}` : '') + `) x LEFT OUTER JOIN (SELECT * FROM kanmi_records_extended) y ON (x.eid = y.eid)) rec LEFT OUTER JOIN (${selectCDN}) cdn ON (rec.eid = cdn.eid)`;
        const selectBaseNoPreLimit = `SELECT rec.*, cdn.host AS cdn_host, path_hint, cdn.full_hint, cdn.preview_hint, cdn.ext_0_hint FROM (SELECT x.*, y.data FROM (SELECT ${sqlFields.join(', ')} FROM ${sqlTables.join(', ')} WHERE (${execute} AND (${sqlWhere.join(' AND ')}))) x LEFT OUTER JOIN (SELECT * FROM kanmi_records_extended) y ON (x.eid = y.eid)) rec LEFT OUTER JOIN (${selectCDN}) cdn ON (rec.eid = cdn.eid)`;
            ...
            if (imageResults && imageResults.rows.length > 0) {
                ...
                randomImage.forEach(function(image) {
                    ...
                    if ( image.cdn_host !== null && config.local_cdn_list.filter(e => e.id === image.cdn_host).length > 0) {
                        ranImage = `${config.local_cdn_list.filter(e => e.id === image.cdn_host)[0].access_url}preview/${image.path_hint}/${image.preview_hint}?${image.attachment_hash}`
                    ...
                    if ((page_uri === '/ambient-refresh' || page_uri === '/ambient-remote-refresh') && !(req.query && req.query.nocds && req.query.nocds === 'true')) {
                        ...
                        if ( image.cdn_host !== null && config.local_cdn_list.filter(e => e.id === image.cdn_host).length > 0) {
                            ...
                            ranfullImage = `${cdn_host}full/${image.path_hint}/${image.full_hint}?${image.attachment_hash}`
                            ...
                            if (index + 1 <= limit) {
                                ...
                                if (item.attachment_extra !== null) {
                                    ...
                                    if ( item.cdn_host !== null && config.local_cdn_list.filter(e => e.id === item.cdn_host).length > 0) {
                                        fullimage = fullimage = imageurl = downloadimage = `${config.local_cdn_list.filter(e => e.id === item.cdn_host)[0].access_url}full/${item.path_hint}/${item.full_hint}?${item.attachment_hash}`
                            ...
                            if (index + 1 <= limit) {
                                ...
                                if (item.attachment_extra !== null) {
                                    ...
                                    if (item.cdn_host !== null && config.local_cdn_list.filter(e => e.id === item.cdn_host).length > 0) {
                                        ...
                                        fullurl = downloadurl = imageurl = `${cdn_host}full/${item.path_hint}/${item.full_hint}?${item.attachment_hash}`
                                    } else {
