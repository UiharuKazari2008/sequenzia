        ...
        if (page_uri === '/start') {
                    ...
                    if ((page_uri === '/ambient-refresh' || page_uri === '/ambient-remote-refresh') && !(req.query && req.query.nocds && req.query.nocds === 'true')) {
                        ...
                        if (image.cdn_host !== null && config.local_cdn_list.filter(e => e.id === image.cdn_host).length > 0 && image.full_hint) {
                            ...
                            if (req.query && req.query.generateImage && req.query.generateImage === 'true') {
                                ranfullImage = `${cdn_host.gen_url}ads-gen/local/${thisUser.master.discord.user.id}/${image.eid}/${image.path_hint}/${image.full_hint}`
                            } else {
                            ...
                            ranfullImage = `${config.local_cdn_list.filter(e => !!e.gen_url)[0].gen_url}ads-gen/discord/${thisUser.master.discord.user.id}/${image.eid}/` + ((image.attachment_hash.includes('/')) ? image.attachment_hash : `${image.channelid}/${image.attachment_hash}/${image.attachment_name}${(image.auth_valid) ? '?' + image.attachment_auth : ''}`)
            ...
            if (req.session.pageinatorEnable && req.session.pageinatorEnable === true && (!req.query || (req.query && !req.query.watch_history))) {
                ...
                if (sqlAlbumWhere.length > 0) {
                    ...
                    favmatch += `AND sequenzia_album_items.eid = kanmi_records.eid AND sequenzia_album_items.aid = sequenzia_albums.aid AND (${sqlAlbumWhere}) AND (sequenzia_albums.owner = '${thisUser.master.discord.user.id}' OR sequenzia_albums.privacy = 0)`;
                } else if (req.query.album_name) {
                    sqlTables.push('sequenzia_album_items');
                    sqlTables.push('sequenzia_albums');
                    sqlCountFeild = 'sequenzia_album_items.date';
                    favmatch += `AND sequenzia_album_items.eid = kanmi_records.eid AND sequenzia_album_items.aid = sequenzia_albums.aid AND sequenzia_albums.name = '${req.query.album_name}' AND (sequenzia_albums.owner = '${thisUser.master.discord.user.id}' OR sequenzia_albums.privacy = 0)`;
