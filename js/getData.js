...
module.exports = async (req, res, next) => {
    ...
    async function writeHistory(title, times) {
        ...
        if (!current_params.has('responseType') && !current_params.has('setscreen') &&
            ...
            if (last.rows.length > 0) {
                ...
                if (noLastTags === noTags) {
                    ...
                    await sqlPromiseSafe(`INSERT INTO sequenzia_navigation_history SET ? ON DUPLICATE KEY UPDATE date = CURRENT_TIMESTAMP, uri = ?, times = ?`, [{
                        index: `${thisUser.master.discord.user.id}-${md5(cleanURL)}`,
                        uri: accessURL,
                        title,
                        user: thisUser.master.discord.user.id,
                        times
                    }, accessURL, times])
                }
            } else {
            ...
            await sqlPromiseSafe(`DELETE a FROM sequenzia_navigation_history a LEFT JOIN (SELECT \`index\` AS keep_index, date FROM sequenzia_navigation_history WHERE user = ? AND saved = 0 ORDER BY date DESC LIMIT ?) b ON (a.index = b.keep_index) WHERE b.keep_index IS NULL AND a.user = ? AND saved = 0;`, [thisUser.master.discord.user.id, 50, thisUser.master.discord.user.id])
        }
    }
    ...
    if (req.session.active_exchange && req.session.active_exchange !== 'master') {
        ...
        // Pinned
        let pinsUser = `${thisUser.master.discord.user.id}`
        ...
        const selectAlbums = `SELECT DISTINCT ${sqlAlbumFields} FROM sequenzia_albums, sequenzia_album_items WHERE (sequenzia_album_items.aid = sequenzia_albums.aid AND (${sqlAlbumWhere}) AND (sequenzia_albums.owner = '${thisUser.master.discord.user.id}' OR sequenzia_albums.privacy != 1))`
        ...
        if (page_uri === '/start') {
            ...
            if (imageResults && imageResults.rows.length > 0) {
                ...
                randomImage.forEach(function(image) {
                    ...
                    let ranImage = '';
                    if ( image.cdn_host !== null && config.local_cdn_list.filter(e => e.id === image.cdn_host).length > 0 && image.preview_hint) {
                        ranImage = `${config.local_cdn_list.filter(e => e.id === image.cdn_host)[0].access_url}preview/${image.path_hint}/${image.preview_hint}`
                        ...
                        ranImage = `${(req.session && req.session.lite_mode === true) ? '/media_attachments' : 'https://media.discordapp.net/attachments'}/` + ((image.attachment_hash.includes('/')) ? image.attachment_hash : `${image.channelid}/${image.attachment_hash}/${image.attachment_name}${(image.auth_valid) ? '?' + image.attachment_auth : ''}`)
                    }
                    ...
                    if ((page_uri === '/ambient-refresh' || page_uri === '/ambient-remote-refresh') && !(req.query && req.query.nocds && req.query.nocds === 'true')) {
                        ...
                        if (req.query && req.query.generateImage && req.query.generateImage === 'true') {
                            ranfullImage = `${(page_uri !== '/ambient-get') ? web.base_url : ''}/wallpaper/${thisUser.master.discord.user.id}/${image.eid}.png`
                            if (req.query.width && req.query.height) {
                                ranfullImage += `?width=${req.query.width}&height=${req.query.height}`
                            }
                        } else if (image.cdn_host !== null && config.local_cdn_list.filter(e => e.id === image.cdn_host).length > 0 && image.full_hint) {
            ...
            if (imageResults && imageResults.rows.length > 0) {
                    ...
                    try {
                        ...
                        if (displayConfig && displayConfig.rows.length > 0) {
                            ...
                            res.json({
                                ...
                                user_id: thisUser.master.user.id,
                ...
                if (_dn !== "*" && (randomImage.length === 1 || (randomImage.length > 1 && req.query.nohistory && req.query.nohistory === 'false')) && thisUser.master.discord.user.id && randomImage && !req.query.displaySlave && !(req.query.nohistory && req.query.nohistory === 'true')) {
                    for (const image of randomImage) {
                        const index = randomImage.indexOf(image);
                        const isExsists = await sqlPromiseSafe(`SELECT * FROM sequenzia_display_history WHERE eid = ? AND user = ?`, [image.eid, thisUser.master.discord.user.id]);
                        if (isExsists.error) {
                            printLine('SQL', `Error adding messages to display history - ${isExsists.error.sqlMessage}`, 'error', err)
                        }
                        if (isExsists.rows.length > 0) {
                            const updateHistoryItem = await sqlPromiseSafe(`UPDATE sequenzia_display_history SET screen = ?, name = ?, date = ? WHERE eid = ? AND user = ?`, [screenID, _dn, moment().format('YYYY-MM-DD HH:mm:ss'), image.eid, thisUser.master.discord.user.id])
                            ...
                            const updateHistoryItem = await sqlPromiseSafe(`INSERT INTO sequenzia_display_history SET eid = ?, name = ?, screen = ?, user = ?, date = ?`, [image.eid, _dn, screenID, thisUser.master.discord.user.id, moment().format('YYYY-MM-DD HH:mm:ss')])
                            if (updateHistoryItem.error) {
                                printLine('SQL', `Error adding messages to display history - ${updateHistoryItem.error.sqlMessage}`, 'error', err)
                            } else {
            ...
            if (req.session.pageinatorEnable && req.session.pageinatorEnable === true && (!req.query || (req.query && !req.query.watch_history))) {
                ...
                if (req.query && req.query.history && req.query.history === 'only') {
                    ...
                    favmatch = `AND sequenzia_display_history.eid = kanmi_records.eid AND sequenzia_display_history.user = '${thisUser.master.discord.user.id}'${(_dn !== '*') ? "  AND sequenzia_display_history.name = '" + _dn.replace(/'/g, '\\\'') + "'" : ''}`;
                }
                if (sqlAlbumWhere.length > 0) {
                    sqlTables.push('sequenzia_album_items');
                    sqlTables.push('sequenzia_albums');
                    sqlCountFeild = 'sequenzia_album_items.date';
                    favmatch += `AND sequenzia_album_items.eid = kanmi_records.eid AND sequenzia_album_items.aid = sequenzia_albums.aid AND (${sqlAlbumWhere}) AND (sequenzia_albums.owner = '${thisUser.master.discord.user.id}' OR sequenzia_albums.privacy != 1)`;
                } else if (req.query.album_name) {
                    sqlTables.push('sequenzia_album_items');
                    sqlTables.push('sequenzia_albums');
                    sqlCountFeild = 'sequenzia_album_items.date';
                    favmatch += `AND sequenzia_album_items.eid = kanmi_records.eid AND sequenzia_album_items.aid = sequenzia_albums.aid AND sequenzia_albums.name = '${req.query.album_name}' AND (sequenzia_albums.owner = '${thisUser.master.discord.user.id}' OR sequenzia_albums.privacy != 1)`;
                ...
                const countResults = await (async () => {
                    if ((await getCacheData(`meta-${thisUser.master.discord.user.id}-${md5(sqlCallNoPreLimit)}`, true)))
                        return { rows: [{ total_count: (await getCacheData(`meta-${thisUser.master.discord.user.id}-${md5(sqlCallNoPreLimit)}`, true)).count } ] };
            ...
            const messageResults = await (async () => {
                ...
                if ((((new Date() - initQuery) / 1000) >= 1.5) && cacheEnabled && reCache &&
                    !(await getCacheData(`lock-${thisUser.master.discord.user.id}-${md5(sqlCallNoPreLimit)}`))) {
                    if (_return.rows.length < sqllimit + 10) {
                        const localKey = `${thisUser.master.discord.user.id}-${crypto.randomBytes(32).toString("hex")}`
                        await setCacheData(`query-${thisUser.master.discord.user.id}-${md5(sqlCallNoPreLimit)}`, {
                            rows: _return.rows,
                        }, true, localKey);
                        await setCacheData(`meta-${thisUser.master.discord.user.id}-${md5(sqlCallNoPreLimit)}`, {
                            time: 300000,
            ...
            if (messageResults && messageResults.rows.length > 0) {
                ((messages) => {
                            ...
                            if (index + 1 <= limit) {
                                ...
                                let post_user = (() => {
                                    ...
                                    if (_u) {
                                        return {
                                            ...
                                            avatar: (_u.avatar) ? `${(req.session && req.session.lite_mode === true) ? '/avatars/' : 'https://cdn.discordapp.com/avatars/'}${item.user}/${_u.avatar}.png?size=512` : null,
                                            system_user: false
                                        }
                                    }
                                    return {
                                ...
                                if (item.attachment_extra !== null) {
                                        ...
                                        if (attachment[0].includes('.jp') || attachment[0].includes('.png') ||
                                            ...
                                            resultsArray.push({
                                                ...
                                                meta: {
                                                    ...
                                                    static_tags: (item.post_tags) ? item.post_tags : [],
                                                ...
                                                server: {
                                                    ...
                                                    icon: `${(req.session && req.session.lite_mode === true) ? '/icons' : 'https://cdn.discordapp.com/icons'}/${item.server}/${item.server_avatar}.png?size=4096`
                                                }
                                            })
                                    ...
                                    if ( item.cdn_host !== null && config.local_cdn_list.filter(e => e.id === item.cdn_host).length > 0 && item.full_hint) {
                                        fullimage = fullimage = imageurl = downloadimage = `${config.local_cdn_list.filter(e => e.id === item.cdn_host)[0].access_url}full/${item.path_hint}/${item.full_hint}?_=${item.id}`
                                    } else if (item.attachment_hash && item.attachment_name) {
                                        fullimage = fullimage = imageurl = downloadimage = `${(req.session && req.session.lite_mode === true) ? '/attachments' : 'https://cdn.discordapp.com/attachments'}/` + ((item.attachment_hash.includes('/')) ? item.attachment_hash : `${item.channel}/${item.attachment_hash}/${item.attachment_name}${(item.auth_valid) ? '?' + item.attachment_auth : ''}`)
                                    } else if (item.fileid) {
                                        fullimage = `/stream/${item.fileid}/${item.real_filename}`
                                    } else if (item.cache_proxy) {
                                        fullimage = fullimage = imageurl = downloadimage = item.cache_proxy.startsWith('http') ? item.cache_proxy : `${(req.session && req.session.lite_mode === true) ? '/media_attachments' : 'https://media.discordapp.net/attachments'}${item.cache_proxy}`
                                    }
                                    if (isCached) {
                                    ...
                                    if (item.fileid !== null && item.cdn_host !== null && config.local_cdn_list.filter(e => e.id === item.cdn_host).length > 0 && item.mfull_hint) {
                                        downloadimage = `${config.local_cdn_list.filter(e => e.id === item.cdn_host)[0].access_url}master/${item.path_hint}/${item.mfull_hint}?_=${item.id}`
                                    } else if (item.fileid) {
                                        downloadimage = `/stream/${item.fileid}/${encodeURIComponent(item.real_filename)}`
                                    } else if (item.attachment_hash && !item.auth_valid && item.cdn_host === null) {
                                        downloadimage = `/file_gateway/${item.channel}/${item.eid}/${item.attachment_name}`
                                    }
                                    if ( item.cdn_host !== null && config.local_cdn_list.filter(e => e.id === item.cdn_host).length > 0 && item.preview_hint) {
                                        imageurl = `${config.local_cdn_list.filter(e => e.id === item.cdn_host)[0].access_url}preview/${item.path_hint}/${item.preview_hint}?_=${item.id}`
                                    } else if (item.cache_proxy) {
                                        ...
                                        imageurl = `${(req.session && req.session.lite_mode === true) ? '/media_attachments' : 'https://media.discordapp.net/attachments'}/` + ((item.attachment_hash.includes('/')) ? `${item.attachment_hash}${getimageSizeParam()}` : `${item.channel}/${item.attachment_hash}/${item.attachment_name}${getimageSizeParam()}${(item.auth_valid) ? '&' + item.attachment_auth : ''}`)
                                    }
                                    let advColor = [];
                                    ...
                                    resultsArray.push({
                                        ...
                                        manage: (thisUser.master.discord.channels.manage.indexOf(item.channel) !== -1)
                                    })
                                    imagesArray.push(imageurl);
                                }
                            }
                            ...
                            if (index + 1 <= limit) {
                                ...
                                if (item.attachment_extra !== null) {
                                    ...
                                    if (item.cdn_host !== null && config.local_cdn_list.filter(e => e.id === item.cdn_host).length > 0 && item.full_hint) {
                                        ...
                                        fullurl = downloadurl = imageurl = `${cdn_host}full/${item.path_hint}/${item.full_hint}`
                                    } else {
                                        fullurl = downloadurl = imageurl = `${(req.session && req.session.lite_mode === true) ? '/attachments' : 'https://cdn.discordapp.com/attachments'}/` + ((item.attachment_hash.includes('/')) ? `${item.attachment_hash}` : `${item.channel}/${item.attachment_hash}/${item.attachment_name}${(item.auth_valid) ? '?' + item.attachment_auth : ''}`)
                                    }
                    ...
                    if (resultsArray.length > 0) {
                        ...
                        if (req.query.slim_results === 'true') {
                            res.locals.response = {
                                ...
                                ultraCache: (messageResults.cache) ? {
                                    id: md5(sqlCallNoPreLimit),
                                    time: messageResults.cache
                                } : false,
                                is_remote_exchange: req.headers['x-sequenzia-exchange'] || false,
                                active_exchange_id: (!req.headers['x-sequenzia-exchange']) ? req.session.active_exchange : 'master'
                            }
                        ...
                        printLine('GetData', `"${thisUser.master.discord.user.username}" => "${page_title}" - ${resultsArray.length} Returned (${_req_uri})`, 'info', {
                            title: page_title,
                            full_title: full_title,
                            page_image,
                            description: description,
