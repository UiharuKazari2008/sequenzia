...
module.exports = async (req, res) => {
    ...
    if (req.query.command) {
            ...
            sqlSafe(`SELECT rec.*, cdn.host AS cdn_host FROM (SELECT * FROM (SELECT * FROM (SELECT sequenzia_display_history.name AS display_name, sequenzia_display_history.screen AS screen_id, sequenzia_display_history.date AS display_date, kanmi_channels.nsfw, kanmi_records.eid, kanmi_records.channel, kanmi_records.server, kanmi_records.filecached,  kanmi_records.fileid,  kanmi_records.real_filename, kanmi_records.cache_proxy, kanmi_records.attachment_hash, kanmi_records.attachment_name, kanmi_records.colorR, kanmi_records.colorG, kanmi_records.colorB, kanmi_records.sizeH, kanmi_records.sizeW, kanmi_records.sizeR FROM kanmi_records, kanmi_channels, sequenzia_display_history WHERE sequenzia_display_history.name != 'PageResults' AND sequenzia_display_history.user = ? AND kanmi_records.eid = sequenzia_display_history.eid AND kanmi_channels.channelid = kanmi_records.channel) x LEFT OUTER JOIN (SELECT name AS config_name, nice_name AS config_nice FROM sequenzia_display_config WHERE user = ?) y ON (x.display_name = y.config_name)) x LEFT OUTER JOIN (SELECT eid AS fav_id, date AS fav_date FROM sequenzia_favorites WHERE userid = ?) y ON x.eid = y.fav_id ORDER BY display_name, display_date DESC) rec LEFT OUTER JOIN (${selectCDN}) cdn ON (rec.eid = cdn.eid)`, [thisUser.master.discord.user.id, thisUser.master.discord.user.id, thisUser.master.discord.user.id], (err, history) => {
                if (err) {
                    ...
                    [
                        ...
                        displayQuery.filter(e => e && e.eid).splice(0,(req.query.json && req.query.json === 'true') ? 75 : 2).forEach((x, index) => {
                            try {
                                ...
                                displayHistory.push({
                                    preview: (x.cdn_host !== null && config.local_cdn_list.filter(e => e.id === x.cdn_host).length > 0) ? `${config.local_cdn_list.filter(e => e.id === x.cdn_host)[0].access_url}preview/${x.server}/${x.channel}/${x.eid}.${((x.cache_proxy) ? x.cache_proxy : (x.attachment_hash.includes('/')) ? x.attachment_hash : x.attachment_name).split('?')[0].split('.').pop()}?${x.attachment_hash}` : ((x.cache_proxy !== null) ? x.cache_proxy : `https://media.discordapp.net/attachments/` + ((x.attachment_hash.includes('/')) ? `${x.attachment_hash}${getimageSizeParam()}` : `${x.channel}/${x.attachment_hash}/${x.attachment_name}${getimageSizeParam()}`)),
                                    full: (x.cdn_host !== null && config.local_cdn_list.filter(e => e.id === x.cdn_host).length > 0) ? `${config.local_cdn_list.filter(e => e.id === x.cdn_host)[0].access_url}full/${x.server}/${x.channel}/${x.eid}.${((x.cache_proxy) ? x.cache_proxy : (x.attachment_hash.includes('/')) ? x.attachment_hash : x.attachment_name).split('?')[0].split('.').pop()}?${x.attachment_hash}` : (x.filecached) ? `/stream/${x.fileid}/${x.real_filename}` :( `https://cdn.discordapp.com/attachments/` + ((x.attachment_hash.includes('/')) ? x.attachment_hash : `${x.channel}/${x.attachment_hash}/${x.attachment_name}`)),
                                    id: x.id,
                                    eid: x.eid,
                                    screen: x.screen_id,
                                    channel: x.channel,
                    ...
                    if (req.query.json && req.query.json === 'true') {
                        ...
                        // SeqHistoryFromHome
                        res.render('display_history_embedded', { displayResults, user: thisUser.master.user, login_source: req.session.login_source, });
                    } else {
                        res.render('display_history', { displayResults, user: thisUser.master.user, login_source: req.session.login_source, });
                    }
                } else {
                    res.render('display_history', { noResults: true, user: thisUser.master.user, login_source: req.session.login_source, } );
                }
            })
        } else
        if (req.query.command === 'getAllNames') {
            sqlSafe('SELECT DISTINCT name FROM sequenzia_display_history WHERE user = ? ORDER BY name DESC', [thisUser.master.discord.user.id], (err, history) => {
                if (err) {
                    res.render('display_history', { user: thisUser.master.user, login_source: req.session.login_source, } );
                    printLine('SQL', `Fail to get display history - ${err.message}`, 'error');
                } else if (history.length > 0) {
                ...
                if (err) {
                    ...
                    history.forEach(x => {
                        ...
                        if ( x.cdn_host !== null && config.local_cdn_list.filter(e => e.id === x.cdn_host).length > 0) {
                            image = `${config.local_cdn_list.filter(e => e.id === x.cdn_host)[0].access_url}preview/${x.server}/${x.channel}/${x.eid}.${((x.cache_proxy) ? x.cache_proxy : (x.attachment_hash.includes('/')) ? x.attachment_hash : x.attachment_name).split('?')[0].split('.').pop()}?${x.attachment_hash}`
        ...
        if (req.query.command === 'timeSync' && req.query.displayname) {
            ...
            sqlSafe('SELECT sequenzia_display_history.date, sequenzia_display_config.refreshTime FROM sequenzia_display_history, sequenzia_display_config WHERE sequenzia_display_history.user = ? AND sequenzia_display_config.user = ? AND sequenzia_display_history.name = ? AND sequenzia_display_config.name = ? AND sequenzia_display_history.screen = ? ORDER BY sequenzia_display_history.date DESC LIMIT 1', [thisUser.master.discord.user.id, thisUser.master.discord.user.id, req.query.displayname, req.query.displayname, screenID], (err, history) => {
                if (err) {
                    res.status(500);
                    printLine('SQL', `Fail to get display history for "${req.query.displayname}" - ${err.message}`, 'error');
                } else if (history.length > 0) {
        ...
        if (req.query.command === 'set' && req.query.displayname && req.query.imageid) {
            const isExsists = await sqlPromiseSafe(`SELECT * FROM sequenzia_display_history WHERE eid = ? AND user = ?`, [req.query.imageid, thisUser.master.discord.user.id]);
            if (isExsists.error) {
                ...
                const updateHistoryItem = await sqlPromiseSafe(`UPDATE sequenzia_display_history SET screen = ?, name = ?, date = ? WHERE eid = ? AND user = ?`, [(req.query.screen) ? parseInt(req.query.screen) : 0, req.query.displayname, moment().format('YYYY-MM-DD HH:mm:ss'), req.query.imageid, thisUser.master.discord.user.id])
                ...
                const updateHistoryItem = await sqlPromiseSafe(`INSERT INTO sequenzia_display_history SET eid = ?, name = ?, screen = ?, user = ?, date = ?`, [req.query.imageid, req.query.displayname, (req.query.screen) ? parseInt(req.query.screen) : 0, thisUser.master.discord.user.id, moment().format('YYYY-MM-DD HH:mm:ss')])
                if (updateHistoryItem.error) {
        ...
        if (req.query.command === 'getConfig' && req.query.displayname) {
            sqlSafe('SELECT * FROM sequenzia_display_config WHERE user = ? AND name = ? LIMIT 1', [thisUser.master.discord.user.id, req.query.displayname], (err, displayConfig) => {
                if (err) {
                    ...
                    if (req.query.json && req.query.json === 'true') {
                        ...
                        res.render('display_config', {
                            configuration: _configuration, user: thisUser.master.user, login_source: req.session.login_source,
                        })
                    }
                } else {
                    if (req.query.json && req.query.json === 'true') {
                        res.json({
                            configuration: {name: req.query.displayname, refreshTime: null}, user: thisUser.master.user, login_source: req.session.login_source,
                        })
                    } else {
                        res.render('display_config', {
                            configuration: {name: req.query.displayname, refreshTime: null}, user: thisUser.master.user, login_source: req.session.login_source,
        ...
        if (req.query.command === 'setConfig' && req.query.displayname) {
            req.body.user = thisUser.master.discord.user.id;
            req.body.name = req.query.displayname;
            if (req.query.update && req.query.update === 'true') {
                sqlSafe('INSERT INTO sequenzia_display_config SET ? ON DUPLICATE KEY UPDATE ?', [{ uid: `${thisUser.master.discord.user.id}-${req.query.displayname}`, ...req.body}, req.body], (err, displayConfig) => {
                    if (err) {
                        res.status(500).send('Internal server error');
                ...
                req.body.uid = `${thisUser.master.discord.user.id}-${req.query.displayname}`;
        ...
        if (req.query.command === 'delete') {
            if (req.query.displayname) {
                sqlSafe('DELETE FROM sequenzia_display_history WHERE user = ? AND name = ?', [thisUser.master.discord.user.id, req.query.displayname], (err, completed) => {
                    if (err) {
                        res.render('display_history', {user: thisUser.master.user, login_source: req.session.login_source, } );
                        printLine('SQL', `Fail to delete display history for "${req.query.displayname}" - ${err.message}`, 'error');
                    } else if (completed.affectedRows > 0) {
                        res.render('display_history', { deleted: completed.affectedRows, user: thisUser.master.user, login_source: req.session.login_source,  } );
                    } else {
                        res.render('display_history', { nothingDeleted: true, user: thisUser.master.user, login_source: req.session.login_source, } );
                    }
                })
                sqlSafe('DELETE FROM sequenzia_display_config WHERE user = ? AND name = ?', [thisUser.master.discord.user.id, req.query.displayname], (err, completed) => {
                    if (err) {
                        printLine('SQL', `Fail to delete display configuration for "${req.query.displayname}" - ${err.message}`, 'error');
                    }
                })
            } else {
                sqlSafe('DELETE FROM sequenzia_display_history WHERE user = ?', [thisUser.master.discord.user.id], (err, completed) => {
                    if (err) {
                ...
                sqlSafe('DELETE FROM sequenzia_display_config WHERE user = ?', [thisUser.master.discord.user.id], (err, completed) => {
                    if (err) {
