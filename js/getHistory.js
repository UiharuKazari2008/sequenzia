                ...
                if (err) {
                    ...
                    history.forEach(x => {
                        ...
                        if ( x.cdn_host !== null && config.local_cdn_list.filter(e => e.id === x.cdn_host).length > 0) {
                            image = `${config.local_cdn_list.filter(e => e.id === x.cdn_host)[0].access_url}preview/${x.server}/${x.channel}/${x.eid}.${((x.cache_proxy) ? x.cache_proxy : (x.attachment_hash.includes('/')) ? x.attachment_hash : x.attachment_name).split('?')[0].split('.').pop()}`
